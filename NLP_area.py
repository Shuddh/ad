...
fillcolors = ['rgba(255, 0, 0, 0.1)', 'rgba(0, 255, 0, 0.1)', 'rgba(0, 0, 255, 0.1)', 'rgba(255, 255, 0, 0.1)',
...
fontfamilies = ["Arial", "Balto", "Courier New", "Droid Sans", "Droid Serif", "Droid Sans Mono", "Gravitas One",
...
markerSymbols = ["circle", "circle-open", "circle-dot", "circle-open-dot", "square", "square-open", "square-dot",
                 ...
                 "triangle-down-open-dot", "triangle-left", "triangle-left-open", "triangle-left-dot",
                 "triangle-left-open-dot", "triangle-right", "triangle-right-open", "triangle-right-dot",
                 "triangle-right-open-dot", "triangle-ne", "triangle-ne-open", "triangle-ne-dot",
                 "triangle-ne-open-dot", "triangle-se", "triangle-se-open", "triangle-se-dot", "triangle-se-open-dot",
                 "triangle-sw", "triangle-sw-open", "triangle-sw-dot", "triangle-sw-open-dot", "triangle-nw",
                 "triangle-nw-open", "triangle-nw-dot", "triangle-nw-open-dot", "pentagon", "pentagon-open",
                 "pentagon-dot", "pentagon-open-dot", "hexagon", "hexagon-open", "hexagon-dot", "hexagon-open-dot",
                 "hexagon", "hexagon-open", "hexagon-dot", "hexagon-open-dot", "octagon", "octagon-open", "octagon-dot",
                 "octagon-open-dot", "star", "star-open", "star-dot", "star-open-dot", "hexagram", "hexagram-open",
                 "hexagram-dot", "hexagram-open-dot", "star-triangle-up", "star-triangle-up-open",
                 "star-triangle-up-dot", "star-triangle-up-open-dot", "star-triangle-down", "star-triangle-down-open",
                 "star-triangle-down-dot", "star-triangle-down-open-dot", "star-square", "star-square-open",
                 "star-square-dot", "star-square-open-dot", "star-diamond", "star-diamond-open", "star-diamond-dot",
                 "star-diamond-open-dot", "diamond-tall", "diamond-tall-open", "diamond-tall-dot",
                 "diamond-tall-open-dot", "diamond-wide", "diamond-wide-open", "diamond-wide-dot",
...
title = "field-gdp-per-capita-ppp"
xaxis_title = "Year"
yaxis_title = "gdp"
...
countries = list(datadict[title]['data'].keys())
shuffle(countries)
values = []
labels = []
# print(datadict[countries])

for i in range(2):
    labels = list(datadict[title][countries[1]].keys())
    yd = []
    for k, v in datadict[title][countries[i]].items():
        yd.append(v)
    values.append(yd)
# values1 = [data[country[year]]]
# values2 = []


trace1 = go.Scatter(
    name=countries[0],
    x=labels if orientation == 'v' else values[0],
    y=values[0] if orientation == 'v' else labels,
    text=values[0] if orientation == 'v' else labels,
    textfont=dict(
        family=fontfamily,
        color=fontcolor,
        size=fontsize
...
trace2 = go.Scatter(
    name=countries[1],
    x=labels if orientation == 'v' else values[1],
    y=values[1] if orientation == 'v' else labels,
    text=values[0] if orientation == 'v' else labels,
    textfont=dict(
        family=fontfamily,
        color=fontcolor,
...
layout = go.Layout(
    ...
    xaxis=dict(
        ...
        ticks='outside',
        tick0=0,
        dtick=1 if orientation == 'v' else getTicks(max(max(values[0]), max(values[1]))),
        ticklen=4,
        tickwidth=1,
        tickcolor='#000',
        mirror=True,
    ...
    yaxis=dict(
        ...
        ticks='outside',
        tick0=0,
        dtick=1 if orientation == 'h' else getTicks(max(max(values[0]), max(values[1]))),
        ticklen=4,
        tickwidth=1,
        tickcolor='#000',
        mirror=True,
